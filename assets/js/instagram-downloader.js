(() => {
 ...
 form.addEventListener('submit',async event=>{
  ...
  try{const url=new URL(form.elements.url.value.trim());if(url.protocol!=='https:'||!['instagram.com','www.instagram.com'].includes(url.hostname)||url.port||url.username||url.password||!/^\/(p|reel)\/[A-Za-z0-9_-]{1,64}\/?$/.test(url.pathname))throw Error();}catch{status.textContent='Paste a valid HTTPS Instagram /p/ or /reel/ URL.';return;}
  ...
  try{
   ...
   result.append(node('h3',media.title));const badge=node('span',media.type==='video'?'Video':'Image');badge.className='badge';result.append(badge);
   ...
   result.append(node('p',media.notice));const button=node('button',media.type==='video'?'Download video':'Download image');button.type='button';button.className='button';result.append(button);
   button.addEventListener('click',async()=>{
    ...
    try{const data=new FormData();data.set('csrf',form.elements.csrf.value);data.set('token',media.token);const blob=await request(form.dataset.downloadUrl,data,true);const ext={'video/mp4':'mp4','video/webm':'webm','image/jpeg':'jpg','image/png':'png','image/webp':'webp','image/gif':'gif'}[blob.type];if(!ext)throw new Error('The server did not return a supported media file.');const href=URL.createObjectURL(blob),link=node('a');link.href=href;link.download=`instagram-${media.type}.${ext}`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(href),60000);status.textContent='Download ready. Check your browser downloads.';}catch(error){status.textContent=message(error);}finally{button.disabled=false;submit.disabled=false;button.textContent=media.type==='video'?'Download video':'Download image';}
