...
(async()=>{
 ...
 try{
  ...
  await page.goto(base+'/admin/no-api-tools');assert.ok(page.url().includes('/login'));
  ...
  for(const id of ['favicon','compress','resize','convert']){await page.locator('#local-tool').selectOption(id);await page.locator('[name=image]').setInputFiles({name:'sample.png',mimeType:'image/png',buffer});if(id==='resize'){await page.locator('[name=width]').fill('200');await page.locator('[name=height]').fill('100');}await page.locator('#local-form [type=submit]').click();await page.waitForFunction(()=>!document.querySelector('#local-download').disabled);const d=await file();if(id==='favicon'){assert.equal(d.bytes.readUInt16LE(2),1);assert.equal(d.bytes.readUInt32LE(18),22);}else{const img=await loadImage(d.bytes);assert.equal(img.width,id==='resize'?200:160);} }
  ...
  await page.setViewportSize({width:390,height:844});assert.equal(await page.evaluate(()=>document.body.scrollWidth>innerWidth+1),false);await page.screenshot({path:'storage/test-no-api-mobile.png',fullPage:true});
  ...
  assert.deepEqual(requests.filter(u=>!u.endsWith('/assets/js/local-minifier.js')),[],'Unexpected tool network request');
  await page.goto(base+'/no-api-tools?website_id=0');assert.equal(await page.locator('#local-tool option').count(),24);assert.equal(await page.locator('.welcome-banner').count(),0);
