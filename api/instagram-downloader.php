...
if($section==='api/v1/instagram-media'){
 ...
 if(!$media||($media['expires']??0)<time())fail('This download link expired. Check the Instagram URL again.',410);
...
$token=bin2hex(random_bytes(24));$_SESSION['instagram_media'][$token]=['url'=>$result['media'],'type'=>$result['type'],'expires'=>time()+600];
