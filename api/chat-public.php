...
$action=substr($section,9);$config=LiveChat::config($widget);
...
if($action==='start'){
 ...
 LiveChat::message($cid,'assistant',$config['welcome']."\n\n".$widget['business_info'].(!$config['online']?"\n\n".$config['offline']:''));db()->commit();json_response(['token'=>$token,'messages'=>LiveChat::history($cid),'mode'=>'bot','lead_captured'=>false]);
}
...
if($action==='message'){
 ...
 if(!value('SELECT id FROM chat_messages WHERE conversation_id=? AND client_id=?',[$cid,$client])){
  ...
  if($conversation['mode']==='bot')LiveChat::message($cid,'assistant',$config['online']?LiveChat::reply(trim($body),rows('SELECT question,keywords,answer FROM chat_knowledge WHERE widget_id=? ORDER BY id',[$widget['id']]),$widget['business_info']):$config['offline']);
...
if($action==='lead'){
 if(!value('SELECT id FROM chat_leads WHERE conversation_id=?',[$cid])){
  $lead=LiveChat::leadInput($data,$config);$score=LiveChat::score($lead);$status='New';
  ...
  if($config['notify'])notify((int)$widget['user_id'],'New chat lead for '.$widget['name'],'/live-chat?tab=lead&id='.$lid);
  query('UPDATE chat_conversations SET updated_at=NOW() WHERE id=?',[$cid]);
 }
}
$leadCaptured=(bool)value('SELECT id FROM chat_leads WHERE conversation_id=?',[$cid]);$messages=LiveChat::history($cid);db()->commit();json_response(['messages'=>$messages,'mode'=>$conversation['mode'],'lead_captured'=>$leadCaptured]);
