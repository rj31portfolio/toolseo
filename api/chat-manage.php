...
if($op==='widget'){
 ...
 $name=required_input('name',120);$info=required_input('business_info',10000);$chatConfig=LiveChat::configInput();$origins=[];
 ...
 $params=[$name,$info,json_encode($chatConfig),json_encode($origins),(int)(enum_input('enabled',['0','1'],'1')==='1')];
...
if(in_array($op,['knowledge','knowledge-delete','automation','automation-delete'],true)){
 ...
 if($op==='knowledge'){
  $values=[required_input('question',300),input('keywords',500),required_input('answer',4000)];
  if($id){if(!value('SELECT id FROM chat_knowledge WHERE id=? AND widget_id=?',[$id,$w['id']]))fail('Knowledge entry not found.',404);query('UPDATE chat_knowledge SET question=?,keywords=?,answer=? WHERE id=? AND widget_id=?',array_merge($values,[$id,$w['id']]));}
  else{if((int)value('SELECT COUNT(*) FROM chat_knowledge WHERE widget_id=?',[$w['id']])>=100)fail('Maximum 100 knowledge entries per widget.');query('INSERT INTO chat_knowledge(question,keywords,answer,widget_id) VALUES (?,?,?,?)',array_merge($values,[$w['id']]));}
 }
 if($op==='knowledge-delete')query('DELETE FROM chat_knowledge WHERE id=? AND widget_id=?',[$id,$w['id']]);
 if($op==='automation'){
  $score=input('min_score',3,'0');if(!ctype_digit($score)||(int)$score>100)fail('Score must be between 0 and 100.');
  $values=[required_input('name',120),input('keyword',120),(int)$score,enum_input('target_status',LiveChat::STATUSES,'Qualified'),(int)(enum_input('enabled',['0','1'],'1')==='1')];
  if($id){if(!value('SELECT id FROM chat_automation_rules WHERE id=? AND widget_id=?',[$id,$w['id']]))fail('Automation rule not found.',404);query('UPDATE chat_automation_rules SET name=?,keyword=?,min_score=?,target_status=?,enabled=? WHERE id=? AND widget_id=?',array_merge($values,[$id,$w['id']]));}
  else{if((int)value('SELECT COUNT(*) FROM chat_automation_rules WHERE widget_id=?',[$w['id']])>=50)fail('Maximum 50 automation rules.');query('INSERT INTO chat_automation_rules(name,keyword,min_score,target_status,enabled,widget_id) VALUES (?,?,?,?,?,?)',array_merge($values,[$w['id']]));}
 }
