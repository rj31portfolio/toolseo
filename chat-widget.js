(() => {
 ...
 function boot(data){
  ...
  restart.addEventListener('click',()=>{if(busy)return;token='';leadCaptured=false;messages=[];log.replaceChildren();showLead(false);try{sessionStorage.removeItem(storageKey);}catch{}open();});
